...
function scaling(obj, scaleFac) {
  const step = obj.speed * 0.0001; // Smaller step for smoother scaling
  const difference = scaleFac - obj.scale;
  if (Math.abs(difference) > step) {
    obj.scale += Math.sign(difference) * step; // Adjust scale incrementally
    return false; // Scaling not complete
  } else {
    obj.scale = scaleFac; // Ensure final value is exact
    return true; // Scaling complete
  }
}


function translation(obj) {
    // Check if deltas are defined; if not, initialize them
    if (typeof obj.deltaX === "undefined" || typeof obj.deltaY === "undefined") {
      setDelta(obj);
    }
    ...
    // Boundary check for bounce (assuming a [-1, 1] viewport range)
    if (obj.trans[0] > 1.0 || obj.trans[0] < -1.0) {
      obj.deltaX = -obj.deltaX; // Reverse X direction
    }
    if (obj.trans[1] > 1.0 || obj.trans[1] < -1.0) {
      obj.deltaY = -obj.deltaY; // Reverse Y direction
    }
  ...
  } // spiral dance, rotate about all axes
  else if (obj.transMode === 4) {
    obj.theta[0] += obj.speed * 0.01;            // slow rotation on x-axis
    obj.theta[1] += obj.speed * 0.015 + 0.005;  // slightly faster rotation on y-axis
    obj.theta[2] += obj.speed * 0.02;           // fastest rotation on z-axis
    obj.scale = 1 + 0.2 * Math.abs(Math.sin(obj.theta[2])); // pulsing scale effect
  }
}
...
function setDelta(obj) {
  ...
  obj.deltaY = obj.speed * Math.sin(Math.PI / 3) * 0.00004;
    ...
    obj.deltaX = obj.speed * Math.cos(angle) * 0.00004;
    obj.deltaY = obj.speed * Math.sin(angle) * 0.00004;
